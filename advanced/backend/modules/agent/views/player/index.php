...
//         'filterModel' => $searchModel,
        'columns' => [
            ...
            ['label'=>'玩家uid','value'=>'account_id'],
            ['label'=>'昵称','value'=>'name'],
            [
            'label'=>'基本信息',
            'format'=>'raw',
                'value'=>function($model) {
                    $ht = html::tag("p","VIP:".$model->power,['class'=>'red']);
                    ...
                    $ht .= html::tag("p",'<i class="icon-time"></i>最后登录:'.date('Y-m-d H:i:s',$model->last_login));
                    return $ht;
                },
            	],
            	[
            	'label'=>'游戏信息',
            	    'format'=>'raw',
            	    'value'=>function($model) {
                	    $ht = html::tag("p","战绩：".$model->playerFlag->win."胜/".$model->playerFlag->lose."负 胜率：".
                	        Yii::$app->formatter->asPercent($model->playerFlag->win==0?0:($model->playerFlag->win/($model->playerFlag->win+$model->playerFlag->lose)),2));
            ...
            'label' => "操作",
                'format' => 'raw',
                'value' => function($model) {
                     $ht = Html::tag('p',Html::a('<i class="icon-gift"></i>充值', "#", ["data-player"=>$model->account_id,'class' => 'gift btn btn-success']) );
                     $ht.=Html::tag('p',Html::a('<i class="icon-edit"></i>详细', "#", ["data-player"=>$model->account_id,'class' => 'mod btn btn-warning'])) ;
