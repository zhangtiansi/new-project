				...
				<form>
					<div class="form-actions">
						<div class="input-group">
							<input placeholder="Type your message here ..." type="text" id="msg-input" class="form-control" name="message"  />
...
//     	var navitem='<div class="list-group-item">用户1</div>';
        	$.ajax({
       		 type: "GET",
       		 url: '/customer/logmsg/unreadlist',
       		 dataType: "json",
...
//     	var navitem='<div class="list-group-item">用户1</div>';
    ...
    function startDialog(fid)
    {
    	$.ajax({
      		 type: "GET",
      		 url: '/customer/logmsg/fetchunread?fid='+fid,
      		 dataType: "json",
      		 success: function(data) {
      			 console.log(data);
      			 if(data.code == 0 ){
    ...
    function tplContent(array,items)
    {
    	for (var i in items){
            ...
            if(item.fid==0){
                ...
                li+='<img src="/images/avatars/'+item.fid+'.jpg" />';
            }
            li+='</div>';
            li+='<div class="body">';
            li+='<div class="time">';
...
// 		append(this.eq(0).clone()).html();
        ...
        if(cont !=""&&dialogfid!=""){
        	...
        	$.ajax({
         		 type: "POST",
         		 url: '/customer/logmsg/sendcust?fid='+dialogfid,
         		 data:{
         		    msg:cont,
         		   _csrf:csrfToken
             		 },
	...
	$("#chat-nav").delegate(".list-group-item","click",function(data){
		...
		var title = '<span>'+fname+'</span><em class="text-muted text-sm">当前金币:'+fmoney+'当前钻石：'+fpoint+'</em>';
