...
$encodingAesKey = "tvNOrLhNvANe5bLc4OoQohHiIts4yyGMeFxJMUmnL9Y";
$token = "U4MCUYPknl";
...
if ($sVerifyEchoStr !=""){
    ...
    if ($errCode == 0) {
    	...
    	// 验证URL成功，将sEchoStr返回
    	echo $EchoStr;
        Yii::info('recieve msg check suc .code == 0 ');
    } else {
        Yii::error('sig check failed '.$errCode);
        print("ERR: " . $errCode . "\n\n");
    }
}else {
        ...
        if ($errCode == 0) {
        	...
        	**/
        	...
        	if ($errCode == 0) {
        	    ...
        	    Yii::error(' response :'.$sEncryptMsg);
        	    echo $sEncryptMsg;
        	} else {
        	    print("ERR: " . $errCode . "\n\n");
        	    // exit(-1);
